...
class Hand(object):
    ...
    def update(self, word):
        ...
        """
        for key in word:
            self.handDict[key] = self.handDict.get(key) - 1

            if self.handDict.get(key) == 0:
                self.handDict.pop(key)
...
class Player(object):
    ...
    def __cmp__(self, other):
        ...
        returns:
            - 1 if this player's score is greater than other player's score
            - -1 if this player's score is less than other player's score
            - 0 if they're equal.
        """
        if float(self.getPoints()) < float(other.getPoints()):
            return -1
...
class Wordlist(object):
    ...
    def __init__(self):
        ...
        with open(WORDLIST_FILENAME) as words_txt:
