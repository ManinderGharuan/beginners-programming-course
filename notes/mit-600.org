...
- Python has two different type of numbers
  - *int*
  - Python unlike some languages called arbitrary precision integers
  - *float*
...
*Lists*: different from string in two ways
  ...
  - *remove*: is the list method using to remove object from the list
...
- We can also assign to a list or to an element of list (e.g ls[1] = 44
  ...
  We cannot assign out of range index

  e.g
  #+BEGIN_SRC python
    ls = ['length', 'of', 'list', 'is', 4]
    ls[5] = 'unknown_index'  #Error because index 5 is not available in list
  #+END_SRC

** Dictionaries
- Dictionaries are also /mutable/ and /heterogeneous/ like lists are (but elements in lists do not have an order)
...
- Find square-root of b*b + h*h
  - Save as float in hypotenuse
- Print out using value in hypotenuse

** Efficiency
- Orders of growth
- Choice of algorithm
- Map a problem into class of algorithms
- There is two things we want to measure: Space and time
  - Space is how much memory need to complete a computation
